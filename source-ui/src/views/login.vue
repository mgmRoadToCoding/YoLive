<template>
  <div class="login">
    <div class="login-logo hidden-md-and-down">
      <span> YoLive | 通行证</span>
    </div>
    <h2 class="main-title"><span>YoLive</span></h2>
    <h3 class="sub-title"></h3>
    <div class="login-box">
      <div class="weixin" v-show="loginType === 0">
        <div class="title">WeChat 로그인</div>
        ...
        <div class="open-weixin">
          ...
          <div class="info">
            <p>공개 번호를 따라가는 WeChat 스캔 코드 자동 로그인</p>
          </div>
        </div>
      </div>
      <div class="pc" v-show="loginType === 1">
        <div class="title">비밀번호 로그인</div>
    ...
    <div class="el-login-footer">
      <span>
        <a href="#" target="_blank"
          >Copyright © 2025-{{ new Date().getFullYear() }} YoLive  All Rights Reserved.</a
        >
      </span>
    </div>
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      loginRules: {
        username: [
          { required: true, trigger: "blur", message: "계좌 번호를 입력하세요." },
        ],
        password: [
          { required: true, trigger: "blur", message: "비밀번호를 입력하세요." },
        ],
        code: [{ required: true, trigger: "change", message: "인증 코드를 입력하세요." }],
      },
      loading: false,
      // 验证码开关
      captchaOnOff: true,
