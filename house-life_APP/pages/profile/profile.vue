<template>
	<view class="u-m-20">
		...
		<u-modal v-model="showModel" @confirm="confirmName" ref="uModal" :async-close="true" title="닉네임 설정">
			<view class="slot-content">
				<u-input v-model="nickName" type="text" :border="false" placeholder="닉네임을 입력해주세요" />
			</view>
		</u-modal>
...
<script>
	...
	export default {
		data() {
			return {
				pic: uni.getStorageSync('lifeData').vuex_user.user.avatar.includes(config.staticUrl) ? uni.getStorageSync(
						'lifeData').vuex_user.user.avatar : config.staticUrl + uni.getStorageSync('lifeData').vuex_user
					.user.avatar,
				nickName: null,
				showModel: false,
			}
		...
		methods: {
			...
			confirmName() {
				if (!this.nickName) {
					...
					return this.$mytip.toast('닉네임을 입력해주세요')
				}
				...
				this.$u.put(url, {
					...
					this.showModel = false;
					this.$mytip.toast('닉네임이 성공적으로 변경되었습니다')
				});
			},
			updateAvatar() {
