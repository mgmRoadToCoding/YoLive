<template>
	<view>
		<u-navbar :is-back="true" title="Change Avatar" :border-bottom="false"></u-navbar>
		<view class="wrap">
			...
			<view class="u-m-t-20 btn">
				<u-button type="primary" @click="subProfile">제출</u-button>
			</view>
		</view>
	</view>
</template>
		...
		methods: {
			subProfile() {
				...
				if (this.$u.test.isEmpty(files)) {
					return this.$mytip.toast('이미지 선택하세요')
				}
				...
				this.$u.put("api/profile/updateProfile", {
					...
					this.$mytip.toast('프로필 사진이 성공적으로 변경되었습니다')
					setTimeout(() => {
